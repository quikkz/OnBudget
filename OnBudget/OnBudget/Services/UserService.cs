...
namespace OnBudget.Services
{
    public class UserService : IDataStore<User>
    {
        ...
        public async Task<IEnumerable<User>> GetItemsAsync(bool forceRefresh = false)
        {
            ...
            return await usersTable.ToEnumerableAsync();
        }
        ...
        public async Task<User> GetItemAsync(string id)
        {
            await InitializeAsync();
            await PullLatestAsync();
            var items = await usersTable.Where(s => s.Id == id).ToListAsync();
        ...
        public async Task<bool> DeleteItemAsync(User item)
        {
            await InitializeAsync();
            await PullLatestAsync();
            await usersTable.DeleteAsync(item);
            await SyncAsync();
        ...
        public async Task<bool> PullLatestAsync()
        {
            ...
            try
            {
                await usersTable.PullAsync($"all{typeof(User).Name}", usersTable.CreateQuery());
            }
            catch (Exception ex)
            {
